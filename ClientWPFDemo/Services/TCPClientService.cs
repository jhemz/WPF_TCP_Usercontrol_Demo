...
{
  public class TCPClientService : IMessageCore
  {
    ...
    private Queue localQueueOfItemsToSendToServer;

    public TCPClientService()
    {
      localQueueOfItemsToSendToServer = new Queue();
      timer = new DispatcherTimer();
      timer.Tick += new EventHandler(TimerTick);
      timer.Interval = new TimeSpan(0, 0, 0, 0, 100);
      backgroundWorker = new BackgroundWorker();
      backgroundWorker.DoWork += new DoWorkEventHandler(SynchroniseClientAndServer);
    }
    ...
    public void AddItemToQueueToSendToServer(KeyValuePair<string, object> keyValuePair)
    {
      localQueueOfItemsToSendToServer.Enqueue(keyValuePair);
    }
    ...
    private void SynchroniseClientAndServer(object sender, DoWorkEventArgs e)
    {
      try
      {
        ...
        Application.Current.Dispatcher.Invoke(() =>
        {
          ...
          while (localQueueOfItemsToSendToServer.Count > 0)
          {
            queueToPassToServer.Enqueue(localQueueOfItemsToSendToServer.Dequeue());
          }
